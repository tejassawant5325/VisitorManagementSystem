...
// files
    ...
    $idfilename = $file['idcarddoc'];
    $idfileerror = $file['error'];
    $idfiletmp = $file['tmp_name'];

    $fileext = explode('.', $idfilename);
    ...
    if (in_array($idfilecheck, $idfileextstored)) {
        $iddestinationfile = 'visitoridcard/' . $idfilename;
        move_uploaded_file($idfiletmp, $iddestinationfile);

    }
    // ----------------------------------------

    $aggfilename = $file['agreement'];
    $aggfileerror = $file['error'];
    $aggfiletmp = $file['tmp_name'];

    $fileext = explode('.', $aggfilename ?? '');
    $aggfileext = explode('.', $aggfilename);
    $filecheck = strtolower(end($aggfileext));
...
Comment,registeredBy,dob,user_image,user_id_card,user_agreement) VALUES ('$public_id','$fname','$mname','$lname','$vmobile','$work','$agentname','$agentno','$ownername','$ownerno','$vcaste','$vreligion','$paddress','$taddress',
'$department','$day', '$month', '$year', '$date',
'$timein','$rid','$comment',
'$user','$dob','$destinationfile','$iddestinationfile','$aggdestinationfile')";
                ...
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu">
                            <li><a class="dropdown-item" href="addvisitor.php">Add Visitor</a></li>
    ...
    <div class="container">
        ...
        <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="ownername" class="form-control" id="ownername" placeholder="Enter Owner"
                    required>
            </div>
            <div class="mb-3">
                <label for="Owner Mobile Number" class="form-label">Owner Mobile Number</label>
                <input type="number" name="ownerno" class="form-control" id="ownerno" pattern="\d*" maxlength="10" minlength="10" placeholder="Enter Owner Mobile Number" required>
            </div>
            <div class="mb-3">
