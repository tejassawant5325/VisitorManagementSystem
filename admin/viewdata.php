...
<body>
    ...
    <div class="container">
        ...
        <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post">
            ...
            <p>Search by Police Sation <input type="radio" name="search" id="department" value="dept"
                    onclick="disable()"></p>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    elseif (isset($_POST["search"]) && $_POST["search"] == "ownername") {
        ...
        $query = "SELECT * FROM `info_visitor` WHERE `owner_name` = '$ownername'";
        $result = mysqli_query($link, $query);
        $count = mysqli_num_rows($result);
    ...
    elseif (isset($_POST["search"]) && $_POST["search"] == "receipt") {
        ...
        $query = "SELECT * FROM `info_visitor` WHERE `ReceiptID` = $receiptid";
        $result = mysqli_query($link, $query);
        $count = mysqli_num_rows($result);
    ...
    elseif (isset($_POST["search"]) && $_POST["search"] == "dept") {
        $department = $_POST['dept'];
        $query = "SELECT * FROM `info_visitor` WHERE `department`='$department'";
        $result = mysqli_query($link, $query);
        $count = mysqli_num_rows($result);
    ...
    elseif (isset($_POST["search"]) && $_POST["search"]=="job") {
        $job = $_POST['job'];
        $query = "SELECT * FROM `info_visitor` WHERE `Purpose` = '$job'";
        $result = mysqli_query($link,$query);
        $count = mysqli_num_rows($result);
...
function headingMake($res)
{
   ...
   <tr>
    ...
    ';
        if ($result['Status'] == 1) {
            echo '<td><a type="button" class="btn btn-success btn-sm name="status">Accepted</a></td>';

            $status_info = true;
        } else {
            echo '<td><a type="button" class="btn btn-warning btn-sm name="status">Pending</a></td>';

            $status_info = false;
        }

    //     if ($result['Status'] == 1) {
    //         echo '<td><a href="userbadge.php?id=' . $result['Serial'] . '" type="button" class="btn btn-success btn-sm">Print Badge</a></td>';
    //     } else {
    //         echo '
    //  <td><button type="button" class="btn btn-warning btn-sm">Id Pending</button></td>';
    //     }
        //  Renew button
        $date_now = date("Y-m-d");
        if ($date_now < '2023-05-31' && $result['Status'] == 1 ) {
            echo '<td><button type="button" class="btn btn-success btn-sm">Valid</button></td>';
        } else {
            // echo '<td><a href="user_profile.php?id=' . $result['Serial'] . '" type="button" class="btn btn-info btn-sm">Renew</a></td>';
            echo '<td><a type="button" class="btn btn-info btn-sm">Renew</a></td>';
        }
        echo '</tr>';
    }
}
