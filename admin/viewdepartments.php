...
<body>
    ...
    <div class="container mt-3 table-responsive">
        <h4 class="text-center mt-3">Police Stations</h4>
        <table class="table">
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                echo '<tr>
            <th scope="row">' . $row['dept_id'] . '</th>
            <td>' . $row['dept_name'] . '</td>';

                if ($row['status'] == 1) {

                    echo '<td><a href="status.php?deptid=' . $row['dept_id'] . '&status=0" type="button" class="btn btn-success btn-sm name="status">Active</a></td>';
                } else {
                    echo '<td><a href="status.php?deptid=' . $row['dept_id'] . '&status=1" type="button" class="btn btn-danger btn-sm name="status">In-Active</a></td>';

                }

                echo '
            <td><a href="deleteuser.php?deptid=' . $row['dept_id'] . '" type="button" class="btn btn-danger btn-sm name="status">Delete</a></td>
            </tr>';
